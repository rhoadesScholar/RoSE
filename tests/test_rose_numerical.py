...
from RoSE.rose import RotarySpatialEmbedding, RoSEMultiHeadAttention, _init_p_nd, _make_log_spaced_frequencies
...
class TestPartialRotation:
    ...
    def test_rotary_ratio_values(self, rotary_ratio: float):
        ...
        # Check output shape
        assert result.shape == x.shape, f"Shape mismatch for rotary_ratio={rotary_ratio}"
        
        # Check dimension calculation
        expected_rotary_dim = (int(dim * rotary_ratio) // (2 * num_heads)) * (2 * num_heads)
    ...
    def test_default_behavior_preservation(self):
        ...
        layer_explicit = RotarySpatialEmbedding(
            ...
            rotary_ratio=1.0, learnable=False
        )
        ...
        torch.testing.assert_close(
            result_default, result_explicit, atol=1e-6,
    ...
    def test_zero_rotation(self):
        ...
        layer = RotarySpatialEmbedding(
            dim=dim, num_heads=num_heads, spatial_dims=spatial_dims, 
            rotary_ratio=0.0, learnable=False
        )
        ...
        torch.testing.assert_close(
            x, result, atol=1e-6,
            msg="Zero rotation should return input unchanged"
        )
    ...
    def test_non_rotated_part_preservation(self):
        ...
        layer = RotarySpatialEmbedding(
            dim=dim, num_heads=num_heads, spatial_dims=spatial_dims, 
            rotary_ratio=0.5, learnable=False
        )
        ...
        if layer.non_rotary_dim > 0:
            ...
            torch.testing.assert_close(
                non_rotated_input, non_rotated_output, atol=1e-6,
    ...
    def test_invalid_rotary_ratio(self):
        ...
        with pytest.raises(AssertionError, match="rotary_ratio must be between 0.0 and 1.0"):
            RotarySpatialEmbedding(
                dim=dim, num_heads=num_heads, spatial_dims=spatial_dims, rotary_ratio=-0.1
            )
        
        # Test rotary_ratio > 1.0
        with pytest.raises(AssertionError, match="rotary_ratio must be between 0.0 and 1.0"):
            RotarySpatialEmbedding(
                dim=dim, num_heads=num_heads, spatial_dims=spatial_dims, rotary_ratio=1.1
            )
    ...
    def test_rotary_dimension_alignment(self):
        ...
        layer = RotarySpatialEmbedding(
            dim=dim, num_heads=num_heads, spatial_dims=2, rotary_ratio=0.3, learnable=False
        )
        ...
        assert layer.rotary_dim == expected_rotary_dim
        assert layer.rotary_dim % (2 * num_heads) == 0  # Must be divisible by 2*num_heads
    ...
    def test_multihead_attention_partial_rotation(self):
        ...
        layer = RoSEMultiHeadAttention(
            dim=dim, num_heads=num_heads, spatial_dims=spatial_dims, 
            rotary_ratio=0.25, learnable=False
        )
        ...
        assert attn.shape == expected_shape, f"Expected shape {expected_shape}, got {attn.shape}"
